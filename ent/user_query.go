...
func (_q *UserQuery) QueryRealm() *RealmQuery {
...
func (_q *UserQuery) QueryMfaMethods() *MfaMethodQuery {
	query := (&MfaMethodClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withMfaMethods:          _q.withMfaMethods.Clone(),
...
func (_q *UserQuery) WithRealm(opts ...func(*RealmQuery)) *UserQuery {
	query := (&RealmClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithMfaMethods(opts ...func(*MfaMethodQuery)) *UserQuery {
	query := (&MfaMethodClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMfaMethods = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withRealm; query != nil {
		if err := _q.loadRealm(ctx, query, nodes, nil,
			func(n *User, e *Realm) { n.Edges.Realm = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withMfaMethods; query != nil {
		if err := _q.loadMfaMethods(ctx, query, nodes,
			func(n *User) { n.Edges.MfaMethods = []*MfaMethod{} },
			func(n *User, e *MfaMethod) { n.Edges.MfaMethods = append(n.Edges.MfaMethods, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadRealm(ctx context.Context, query *RealmQuery, nodes []*User, init func(*User), assign func(*User, *Realm)) error {
...
func (_q *UserQuery) loadMfaMethods(ctx context.Context, query *MfaMethodQuery, nodes []*User, init func(*User), assign func(*User, *MfaMethod)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mfamethod.FieldUserID)
	}
	query.Where(predicate.MfaMethod(func(s *sql.Selector) {
...
func (_q *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withRealm != nil {
			_spec.Node.AddColumnOnce(user.FieldRealmID)
		}
	}
