...
func (_q *TokenQuery) QuerySession() *SessionQuery {
...
func (_q *TokenQuery) QueryClientApp() *ClientAppQuery {
...
func (_q *TokenQuery) WithSession(opts ...func(*SessionQuery)) *TokenQuery {
	query := (&SessionClient{config: _q.config}).Query()
...
func (_q *TokenQuery) WithClientApp(opts ...func(*ClientAppQuery)) *TokenQuery {
	query := (&ClientAppClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Token, error) {
	...
	if query := _q.withSession; query != nil {
		if err := _q.loadSession(ctx, query, nodes, nil,
			func(n *Token, e *Session) { n.Edges.Session = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withClientApp; query != nil {
		if err := _q.loadClientApp(ctx, query, nodes, nil,
			func(n *Token, e *ClientApp) { n.Edges.ClientApp = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TokenQuery) loadSession(ctx context.Context, query *SessionQuery, nodes []*Token, init func(*Token), assign func(*Token, *Session)) error {
...
func (_q *TokenQuery) loadClientApp(ctx context.Context, query *ClientAppQuery, nodes []*Token, init func(*Token), assign func(*Token, *ClientApp)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Token)
	for i := range nodes {
...
func (_q *TokenQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSession != nil {
			_spec.Node.AddColumnOnce(token.FieldSessionID)
		}
		if _q.withClientApp != nil {
			_spec.Node.AddColumnOnce(token.FieldClientRefID)
		}
	}
