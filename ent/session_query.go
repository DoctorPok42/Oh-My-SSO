...
func (_q *SessionQuery) QueryRealm() *RealmQuery {
...
func (_q *SessionQuery) QueryTokens() *TokenQuery {
...
func (_q *SessionQuery) WithRealm(opts ...func(*RealmQuery)) *SessionQuery {
	query := (&RealmClient{config: _q.config}).Query()
...
func (_q *SessionQuery) WithTokens(opts ...func(*TokenQuery)) *SessionQuery {
	query := (&TokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withRealm; query != nil {
		if err := _q.loadRealm(ctx, query, nodes, nil,
			func(n *Session, e *Realm) { n.Edges.Realm = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withTokens; query != nil {
		if err := _q.loadTokens(ctx, query, nodes,
			func(n *Session) { n.Edges.Tokens = []*Token{} },
			func(n *Session, e *Token) { n.Edges.Tokens = append(n.Edges.Tokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SessionQuery) loadRealm(ctx context.Context, query *RealmQuery, nodes []*Session, init func(*Session), assign func(*Session, *Realm)) error {
...
func (_q *SessionQuery) loadTokens(ctx context.Context, query *TokenQuery, nodes []*Session, init func(*Session), assign func(*Session, *Token)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(token.FieldSessionID)
	}
	query.Where(predicate.Token(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.TokensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *SessionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withRealm != nil {
			_spec.Node.AddColumnOnce(session.FieldRealmID)
		}
		if _q.withOwner != nil {
